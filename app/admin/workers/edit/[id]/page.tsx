...
const totalHours = filteredReports.reduce(
  ...
  <input
    ...
    className="w-full rounded-xl bg-zinc-900 border border-zinc-700 p-4 text-4xl font-bold text-yellow-400"
  />
</div>
  ...
  className="w-full rounded-xl bg-zinc-800 border border-zinc-700 p-3 mb-3"
/>
        <input
  value={worker.email}
  onChange={(e) =>
...
/>
  ...
  <select
    value={worker.team}
    onChange={(e) =>
      setWorker({ ...worker, team: Number(e.target.value) })
    }
...
<button
  onClick={saveWorker}
  className="mt-6 w-full rounded-xl bg-yellow-500 p-4 text-xl font-bold text-black hover:bg-yellow-400"
>
